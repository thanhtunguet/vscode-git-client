...
### Fixed
- **Filter Graph — "Edit Commit Message..." crash** — editing a commit's message threw `Cannot read properties of undefined (reading 'trim')` because the commit-lookup fetched a single commit's metadata via the same code path as branch filtering, which never matches a raw commit SHA and silently returned no data. It now resolves an exact commit SHA directly instead of falling through to branch/remote name matching, and the edit-message command shows a clear error instead of crashing if the lookup still fails.
...
### Added
- **Commit context menu — Shift+Click to reverse copy order** — in Filter Graph and Compare Branches, holding `Shift` while clicking `Copy Commit Hash(es)` or `Copy Commit Message(s)` on a multi-commit selection copies the hashes/messages in reverse order.
...
- **`CommandController` domain command groups** — the 192 handler/helper members previously flattened directly onto `CommandController` are now additionally exposed through 12 grouped facades (`controller.branch`, `.tag`, `.stash`, `.graph`, `.commitDiffView`, `.compare`, `.operations`, `.patch`, `.staging`, `.remote`, `.submodule`, `.worktree`), one class per Git domain in `src/commands/commandController/groups/`. All existing flat members are unchanged and still work — this is purely additive so `activate()`, `register()`, and existing tests required no changes. See `docs/superpowers/plans/2026-07-30-split-command-controller.md` for the grouping rationale and the deferred follow-up (migrating callers off the flat members).
...
- **Commit context menu — multi-commit labels** — in Filter Graph and Compare Branches, the commit context menu actions that support a multi-commit selection (`Open Details`, `Copy Commit Hash`, `Copy Commit Message`, `Create Patch...`, `Cherry-Pick`, `Revert Commit`) now relabel themselves with the plural form and selection count (e.g. `Revert Commit` → `Revert 3 Commits`) when more than one commit is selected, instead of always showing the singular label.
