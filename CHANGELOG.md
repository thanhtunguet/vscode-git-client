...
### Fixed
- **Filter Graph — Commit ID message filter prioritization** — entering a commit hash (a short or full SHA, at least 4 hex characters) into the Message filter field in Filter Graph view now prioritizes direct commit ID resolution from Git history. Even if the target commit is older than the initial 200-commit window, the extension retrieves and displays the commit directly instead of missing it due to pagination limits.
...
- **Force SSH Pull** — new `vscodeGitClient.git.sshPull.github` / `.gitlab` / `.bitbucket` / `.custom` commands (also in Quick Git Actions) pick a remote, rewrite its HTTPS URL to the matching SSH form for GitHub, GitLab, Bitbucket, or a typed custom host, then pull over SSH.
